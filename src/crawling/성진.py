...
# 3️⃣ JSON 로드
# =====================================
json_path = r"C:\Users\user\OneDrive\Desktop\projectcode\크롤링 완료 파일\result_립_아이리무버.json"
output_path = r"C:\Users\user\OneDrive\Desktop\projectcode\processed_reviews_word2vec.json"
...
for p_idx, product in enumerate(tqdm(raw["data"], desc="제품 처리 중")):
    for r_idx, review in enumerate(product["reviews"]["data"]):
        tokens = preprocess_pipeline(review["full_text"], stopwords)
        review["tokens"] = tokens
        review["label"] = sentiment_label(review["score"])
...
for idx, (p_idx, r_idx) in enumerate(review_indices):
    tokens = raw["data"][p_idx]["reviews"]["data"][r_idx]["tokens"]
    vectors = [w2v_model.wv[t] for t in tokens if t in w2v_model.wv]
    if vectors:
        avg_vector = np.mean(vectors, axis=0).tolist()
    else:
        avg_vector = []
    raw["data"][p_idx]["reviews"]["data"][r_idx]["Word2Vec"] = avg_vector
